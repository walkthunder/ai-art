...
Page({
  ...
  async handleGenerate() {
    ...
    try {
      ...
      if (err.errorCode === 'INSUFFICIENT_USAGE' || 
        ...
        console.log('[TransformTemplate] 余额不足，显示充值引导弹窗');
  ...
  async onPaymentSuccess() {
    console.log('[TransformTemplate] 支付成功，自动继续生成');
    ...
    while (retryCount < maxRetries && !usageIncreased) {
      ...
      try {
        ...
        console.log('[TransformTemplate] 轮询次数:', {
          retry: retryCount + 1,
        ...
        if (currentUsageCount > beforeUsageCount) {
          usageIncreased = true;
          console.log('[TransformTemplate] 次数已增加，继续生成');
          break;
        }
      } catch (err) {
        console.error('[TransformTemplate] 查询次数失败:', err);
      }
