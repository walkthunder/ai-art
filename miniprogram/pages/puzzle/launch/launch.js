...
Page({
   ...
   */
  handleGetMore() {
    console.log('[PuzzleLaunch] 点击获取更多次数');
  ...
  onPaymentComplete(e) {
    ...
    this.loadUsageCount().then(() => {
      console.log('[PuzzleLaunch] 余额刷新成功，准备跳转');
      this.navigateToUpload();
    }).catch((err) => {
      console.error('[PuzzleLaunch] 余额刷新失败，但仍然跳转', err);
      // ✅ 即使刷新失败也跳转，让用户可以继续使用
      this.navigateToUpload();
    });
  },
  ...
  navigateToUpload() {
    ...
    setTimeout(() => {
      wx.navigateTo({
        url: '/pages/puzzle/upload/upload',
        fail: (err) => {
          console.error('[PuzzleLaunch] 跳转上传页失败:', err);
          wx.showToast({
            title: '页面跳转失败，请重试',
            icon: 'none',
            duration: 2000
